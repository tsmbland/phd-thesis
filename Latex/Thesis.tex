...
% Structure: keep generic at this stage. Do four species model first. Show that nonlinearity can exist and peaks at 2. Occurs in regions where membrane is dimeric and cytoplasmic monomeric. Then use two species model to demonstrate why this is the case. Conclusion: can get nonlinearity if dimerisation and membrane binding kinetics support dimerisation on the membrane but not cytoplasm


\subsection{Model description}

There are four potential species to consider: membrane monomers, membrane dimers, cytoplasmic monomers and cytoplasmic dimers (\cref{fig:thermodynamic_model_species}A). We can describe chemical potentials for the four species as:
\begin{align}
\mu_{c,1} &= \ln(c_{c,1})\\
\mu_{c,2} &= \ln(c_{c,2}) - w_d\\
\mu_{m,1} &= \ln(c_{m,1}) - w_m\\
\mu_{m,2} &= \ln(c_{m,2}) - 2w_m - w_d
\end{align}

where $w_d$ represents the dimerisation energy and $w_m$ the membrane binding energy. To keep the model simple, we are assuming that a molecule in a dimer has twice the membrane binding energy of monomer (as dimers have two binding interfaces, one for each monomer), and that the dimerisation energy is equal in the membrane and cytoplasm. Thermodynamic equilibrium is reached when the chemical potentials of all species are equal:
\begin{equation}
...
% To assess how well the membrane-binding kinetics of wild type and RING-mutant PAR-2 can be captured by this model, I performed a quantitative comparison to in vivo data of cytoplasmic vs membrane concentrations. 
...
% Discuss the fact that we have SEC MALS data for RING and we might be able to get energies from this
